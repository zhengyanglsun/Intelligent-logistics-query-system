<template>
  <div>
    <h2 class="title">  
          物流信息增加表
    </h2>  
    <p>
      <button class="button1" @click="back_signal()">
        <router-link style="text-decoration:none;color:white" to="/manage/products/info" active-class="active">
          返回上页
        </router-link>
      </button>
    </p>
    <table>
      <tr>
        <th>订单号</th>
        <td>
          <input type="text" v-model="id" />
        </td>
      </tr>
      <tr>
        <th>物品类型</th>
        <input type="text" v-model="type" />
      </tr>
      <tr>
        <th>Vip</th>
        <input type="text" v-model="vip" />
      </tr>
      <tr>
        <th>目的地</th>
        <input type="text" v-model="destination" />
      </tr>    
      <tr>
        <th>到达时间</th>
        <input type="text" v-model="arrival_time" />
      </tr>
    </table>
    <p>
      <button class="button1" @click="btn_add_stuinfo">添加并保存</button>
    </p>
  </div>
</template>

<script>

export default {
  name: "StudentInsert",
  data() {
    return {
      msg: "欢迎来到物流信息管理系统",
      id: "",
      type: "",
      vip: "",
      destination: "",
      arrival_time: "",
      logistics_routes: "",
      deliver: "",
    };
  },


  methods: {
    back_signal: function()  {
      console.log("signal")
      this.$axios({
            method:"post",
            url:"http://localhost:3030/back_signal",
            data: {},
        }).then((res)=>{
            console.log("成功向后端发送post请求:");
            console.log(res.data);
        }).catch(function (error) {
          console.log(error);
        })
    },

    btn_add_stuinfo: function () {
      let products = {
        //前端输入5个数据,需要发送给后端
        id: String(this.id),
        type: Number(this.type),
        vip: Number(this.vip),
        destination: this.destination,
        arrival_time: this.arrival_time,
      };
      console.log("id:",this.id)
      // 判断输入的学生成绩信息是否符合要求
      if(String(products.id).length==0||String(products.type).length==0||String(products.vip).length==0||String(products.destination).length==0||String(products.arrival_time).length==0){
          alert("输入框不能为空，请重新输入");
      }
      else{
            if(products.id.length==5&&String(products.type).length==1&&String(products.vip).length==1&&String(products.arrival_time).length==12)
            {
                // 发送请求给后端

                this.$axios({
                  method:"post",
                  url:"http://localhost:3030/insert_product",
                  data:products,
                }).then((res)=>{
                    console.log("成功向后端发送post请求:");
                    console.log(res.data);
                })
                  .catch(function (error) {
                  // 处理错误情况
                  console.log(error);
                  //  this.msg1="error";
                })
                // //转到主页
                this.$router.push({ path: "/manage/products/info" });
            }
            if(products.id.length!=5){
              alert("订单号应该为5位数字,请重新输入");
            }
            // 如果类型不在1到4中，则提示用户
            if(products.type!=1&&products.type!=2&&products.type!=3&&products.type!=4){
              alert("物品类型应该为1到4之间,请重新输入");
            }
            // 如果vip不是0和1，则提示用户
            if(products.vip!=0&&products.vip!=1){
              alert("Vip应该为0或1,请重新输入");
            }
            if(products.arrival_time.length!=12){
              alert("到达时间应该为12位数字,请重新输入");
            }
      } 
    },
  },
};
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>
h3,
h4 {
  font-weight: normal;
}
ul {
  list-style-type: none;
  padding: 0;
}
li {
  display: inline-block;
  margin: 0 10px;
}
a {
  color: #42b983;
}
input {
  width: 128px;
}
p {
  font-style: normal;
}
p.p-right {
  font-style: italic;
  text-align: right;
}
div.divCen {
  margin: 2px auto;
  width: auto;
}
table {
  width: 60%;
  margin: auto;
  border-collapse: collapse;
  border: 0;
  text-align: center;
}
th {
  /* background-color: #93daff; */
  background: -webkit-linear-gradient(left, #25c481, #25b7c4);
  background: linear-gradient(to right, #25c481, #25b7c4);
  color: #ffffff;
}
th,
td {
  font-size:1em;
  text-align: center;
  padding: 3.8px;
  border-collapse: collapse;
}
th,
td {
  border: 1px solid #c1e9fe;
  border-width: 1px 0 1px 0;
}
tr {
  border: 1px solid #c1e9fe;
}
tr:nth-child(odd) {
  /* background-color: #dbf2fe; */
  background: -webkit-linear-gradient(left, #25c481, #25b7c4);
  background: linear-gradient(to right, #25c481, #25b7c4);
}
tr:nth-child(even) {
  /* background-color: #fdfdfd; */
  background: -webkit-linear-gradient(left, #25c481, #25b7c4);
  background: linear-gradient(to right, #25c481, #25b7c4);
}

</style>